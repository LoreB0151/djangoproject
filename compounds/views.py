...
def computed_mw_new(request):
    if request.method == "POST":
        form, compound = MWForm(request.POST)
        compound_InChIKey = compound.InChIKey
        if form.is_valid():
            post = form.save(commit=False)
            post.calculated_molecular_weight = str(MWcalculator(post.molecular_formula))
            post.pubchem_request = request.get("https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/inchikey/"+compound_InChIKey+"/property/MolecularWeight/json").json
            post.pubchem_request_MW = post.pubchem_request['MolecularWeight']
